...
if (typeof gemius_cookie == "undefined") {
	gemius_cookie = {
		...
		all_exists : function(keys) {
			try {
				var keys_dict = {};
				var found_count = 0;
				for (var i=0; i<keys.length ; i++) {
					keys_dict[keys[i]] = true;
				}
				var cookies = document.cookie.split(';');
				var cookie_arr, cookie_name;
				for (var j=0; j<cookies.length ; j++) {
					cookie_arr = cookies[j].split('=');
					if (cookie_arr.length < 2) {
						continue;
					}
					cookie_name = cookie_arr[0].replace(/^\s+|\s+$/g,'');
					if (keys_dict[cookie_name]) {
						found_count++;
						delete keys_dict[cookie_name];
						if (found_count === keys.length) {
							return true;
						}
					}
				}
...
if (typeof GemiusHitcolConnector == "undefined") {
	GemiusHitcolConnector = function(is_gde) {
		...
		this.fpdbg = "";
		this.fpsrc = "";
		this.rudata = {ruid: "", refreshts: 0, prefix: "", type: "", cached: 0, firstto: 0, refresh_interval: 10, rudata_tail: "", reset: 0};
		this.rudbg = {err: "", ini_ruid: "", ini_type: "", get_type: "", diff: 0};
		this.gdprforgetts = NaN;
		this.privacy = {locked: false, optout: 0, forgetts: 0};
		this.gdpr_state_change = {optout: null, optoutts: null, forgetts: null};
		...
		this.rucookie_name = {nsec: '__gfp_ruid', sec: '__gfp_s_ruid'};
		this.rucookie_pub_name = {nsec: '__gfp_ruid_pub', sec: '__gfp_s_ruid_pub'};
		this.gdpr_cookie_name = {nsec: '__gfp_gdpr', sec: '__gfp_s_gdpr'};
		this.cookie_expire = 34128000000;
		this.rucookie_expire = 34128000000;
		this.gdpr_cookie_expire = 157680000000;
		this.ref = {override: null, ref: "", frametype: 0, topref: 0};
		this.tfne = 0;
		this.tfp = {fpdata: "", origin: "", fixes: {}, interval: 0.5, need_sync: 0, need_fix: 0};
		this.truid = {ruid: "", status: "", interval: 0.15, need_sync: 0};
		this.gdprdisabled = 0;
		this.gdprdata = [];
		...
		this.timeouts = {initto: null, cmpto: null, sto: null, fto: null, tfpto: null, tfplto: null, chto: null,
			ruidto: null, truidlto: null, rudatato: null};
		this.init_called = 0;
		this.started = 0;
		this.visapi_s = "";
		this.visapi_h = "";
...
var check_same_origin = function(ref, page_location) {
	...
	return null;
		...
		this.sendhit = function(robj,nr,consent,lsadd) {
			...
			if (consent && this.truid.status != 'no-consent' && this.rudata.ruid && this.rudata.ruid[0] != '-') {
				params += '&rudata=' + this.getrudataencoded();
			}
			params += this.has_tfp_answer() ? this.ref_params() : robj.refs;
			params += this.inner_params(robj.inner);
		...
		this.sendhits = function(lsadd) {
			if (this.waiting_on_prerender==0) {
				...
				if (this.waiting_for_fpdata==0 && this.waiting_for_lsdata==0 && this.waiting_for_rudata==0) {
		...
		this.latehits = function() {
			...
			if ((this.has_consent === false || (this.waiting_for_fpdata==0 && this.waiting_for_lsdata==0 && this.waiting_for_rudata==0)) && this.waiting_on_prerender==0 && this.waiting_for_chints==0 && this.waiting_for_tfp==0) {
		...
		this.unloadhit = function(robj,nr,consent) {
			...
			params += this.has_tfp_answer() ? this.ref_params() : robj.refs;
			params += this.inner_params(robj.inner);
			var ruconsent = (consent && this.truid.status != 'no-consent');
			if (ruconsent && this.rudata.ruid && this.rudata.ruid[0] != '-') {
				params += '&rudata=' + this.getrudataencoded();
			} else {
				params += '&rudata=-UNLOAD';
			}
			if (consent!==true) {
				params += '&nc=1';
			} else if (robj.explicit===true) {
		...
		this.unload_prepare = function() {
			if (!this.check_privacy_limits()) {
				this.has_consent = false;
				this.require_consent_info = 0;
			} else if (this.require_consent_info) {
				this.has_consent = null;
		...
		this.parse_fpdata = function() {
			...
			var prefix = (arr.length>=4) ? arr[3] : '';
			if (this.rudata.prefix != prefix) {
				if (prefix == '') {
					this.set_rudata('-TURNEDOFF');
				} else {
					this.rudata.cached = 0;
					this.rudata.firstto = 0;
				}
			}
			this.rudata.prefix = prefix;
		};
		...
		this.restore_fpcookie = function() {
			...
			if (this.rudata.cached && (window.self === window.top || this.ref.frametype == 2)) {
				if (!gemius_cookie.all_exists([this.rucookie_name.nsec, this.rucookie_pub_name.nsec])) {
					this.setrucookie();
				}
			}
		};
		this.refresh_rucookie = function() {
			if (this.rudata.cached && (window.self === window.top || this.ref.frametype == 2)) {
				this.getrucookie();
				this.setrucookie();
			}
		...
		this.setfpcookie = function() {
			if (this.check_privacy_limits()) {
				this.setfpcookie_impl();
			}
		};
		this.setfpcookie_impl = function() {
		...
		this.reset_fpcookie_secondary_if_needed = function() {
			if (this.check_privacy_limits() && this.fpcookie_secondary_needs_reset()) {
		...
		this.msgreceiver = function(e) {
			try {
				...
				if (e.source === window.top) {
					...
					if (is_event("_xx_gemius_putruid_xx_")) {
						var arr = e.data.substr(23).split("/");
						if (arr.length>=2 && arr[1]!='') {
							this.truid.ruid = (arr[0]!='')?arr[0]:null;
							this.truid.status = arr[1];
							this.parse_rudata(arr[2]||arr[0]);
							this.truid.need_sync = 0;
							this.try_and_call_rudata_callbacks();
							this.latehits();
						}
					}
				...
				if (window.top === window.self) {
					...
					if (is_event("_xx_gemius_getruid_xx_/")) {
						var id = e.data.substr(23).split("/")[0];
						var clbk = function(ruid,status,rudata) {
							var msg = "_xx_gemius_putruid_xx_/" + (ruid||"") + "/" + status + "/" + rudata;
							e.source.postMessage(msg, e.origin);
						};
						if (id!='' && !(id in this.rudata_frame_callbacks)) {
							this.rudata_frame_callbacks[id] = clbk;
							this.try_and_call_rudata_callbacks();
						}
					}
				}
...
// cmp_utils.js 
		...
		this.inner_params = function(req_inner) {
			if (typeof encodeURIComponent != 'undefined') {
				...
				if (this.rudbg.err)	{
					inner += '|_rd_err=' + this.rudbg.err.substring(0,30);
				}
				if (this.rudbg.ini_type!='') {
					inner += '|_rd_ini=' + this.rudbg.ini_type;
				}
				if (this.rudbg.get_type!='') {
					inner += '|_rd_get=' + this.rudbg.get_type;
				}
				if (this.rudbg.diff) {
					inner += '|_rd_diff=' + this.rudbg.diff;
				}
				if (this.fpsrc) {
					inner += '|_fpsrc=' + this.fpsrc;
				}
				if (this.prend_flags) {
					inner += '|_prendf=' + this.prend_flags;
				}
		...
		this.internal_hit = function(allowaddscript,vers,id,evid,et,hsrc,sonar,extra,refs,href,inner) {
			var req = "";
			...
			var robj = {req:req,id:id,allowaddscript:allowaddscript,vers:vers,explicit:this.explicit_consent,refs:refs,inner:inner};
		...
		this.consent_loaded = function(consent,explicit) {
			if (this.check_privacy_limits()) {
				this.consent_loaded_impl(consent,explicit);
			}
		};
		this.consent_loaded_impl = function(consent,explicit) {
		...
		this.consentto = function() {
			if (this.waiting_for_consent==1) {
				...
				this.has_consent = false;
				this.latehits();
				this.call_rudata_callbacks(null, "no-consent");
			}
		};
		...
		this.tfp_pageshow = function() {
			...
			if (this.timeouts.truidlto != null) {
				this.truid.interval = 0.15;
				this.timeouts.truidlto.time = 0.15;
			}
		};
		...
		this.truid_loop = function() {
			if (this.truid.need_sync) {
				window.top.postMessage("_xx_gemius_getruid_xx_/" + this.exid ,"*");
				this.timeouts.truidlto = this.set_timeout(this.bind(this.truid_loop), this.truid.interval * 1000);
				if (this.truid.interval < 8) {
					this.truid.interval *= 2;
				}
				if (this.truid.interval >= 8 && this.truid.status=='') {
					this.truid.ruid = null;
					this.truid.status = "no-top";
					this.try_and_call_rudata_callbacks();
				}
				return;
			}
		...
		this.get_id = function(id) {
			if (!id && this.event_identifier != null) {
				return this.event_identifier;
			}
			return id;
		};
		this.gcmdevent = function(obj) {
			if (typeof obj != 'object') return;
			var id = this.get_id(obj.id);
			if (!id || typeof id != 'string') return;
			var et = typeof obj.type == 'string' ? obj.type : "view";
			var extra = this.array_to_string(obj.extra, 0);
			var inner = this.array_to_string(obj.inner, 0);
			this.internal_hit(0,107,id,0,et,3,this.sonar_auto_init,extra,null,null,inner);
		};
...
// chints_params.js 
	...
	return params;
		...
		this.load_fpdata = function() {
			if (this.waiting_for_consent==0 && this.load_fpdata_started==0) {
				...
				if (domain == hcdomain || domain.substr(-hcdomain.length-1) == ("."+hcdomain)) {
					...
					url += (this.privacy.forgetts>0)?('&v='+this.privacy.forgetts):'';
		...
		this.load_fpcdomain = function() {
			if (this.load_fpcdomain_started==0 && this.load_fpdata_started==0) {
				this.load_fpcdomain_started = 1;
				var domain = new String(document.location.hostname);
				var hcdomain = "hit.gemius.pl";
				if (domain == hcdomain || domain.substr(-hcdomain.length-1) == ("."+hcdomain)) {
					this.fpcdomain = '';
					this.fpcdomain_loaded();
				} else {
					var url = (this.hssl?'https://':'http://')+this.hc+'/fpcdomain.js?href='+domain;
					this.append_script(url,this.bind(this.fpcdomain_loaded),0);
				}
			}
		};
		this.fpcdomain_loaded = function() {
			this.try_and_save_gdpr_state();
		};
		this.check_privacy_limits = function() {
			try {
				var changed = false;
				var state = '';
				var gdpr_state = this.get_gdpr_state();
				if (gdpr_state.forgetts>0) {
					var fpdatats = _get_fpdata_ts(this.fpdata)/1000;
					this.privacy.forgetts = gdpr_state.forgetts;
					if (fpdatats>0 && fpdatats<gdpr_state.forgetts) {
						changed = true;
						state = '-FORGOTTEN';
					}
				}
				if (this.privacy.optout != gdpr_state.optout) {
					changed = true;
					if (gdpr_state.optout) {
						state = '-OPTOUT';
					}
				}
				if (state != '' && this.init_called == 1) {
					this.privacy.optout = gdpr_state.optout;
					this.consent_loaded_impl(false, true);
					if (this.fpcdomain != '') {
						this.fpdata = state;
						this.setfpcookie_impl();
					}
					if (this.rudata.ruid && this.rudata.ruid[0] != '-' && (window.self === window.top || this.ref.frametype == 2)) {
						var now = Math.floor((new Date()).getTime()/1000);
						this.set_rudata_impl(state+"|"+now);
					}
				}
				this.privacy.locked = this.privacy.locked || changed;
			} catch (e) {}
			return !this.privacy.locked;
		};
		this.get_gdpr_state = function() {
			var optout = 0;
			var optoutts = 0;
			var forgetts = 0;
			var gdpr_cookie = gemius_cookie.get_on_state(this.gdpr_cookie_name, this.fpdata_sec_state);
			if (gdpr_cookie.sec_state!='unknown') {
				if (this.fpdata_sec_state=='unknown') {
					this.fpdata_sec_state = gdpr_cookie.sec_state;
				}
				var arr = gdpr_cookie.primary.split('|');
				optout = (arr[0] == '1')?1:0;
				optoutts = (arr.length>1)?(parseInt(arr[1])||0):0;
				forgetts = (arr.length>2)?(parseInt(arr[2])||0):0;
			}
			return {optout: optout, optoutts: optoutts, forgetts: forgetts};
		};
		this.set_gdpr_cookie = function(gdpr_state) {
			gemius_cookie.set(this.gdpr_cookie_name, gdpr_state.optout+"|"+gdpr_state.optoutts+"|"+gdpr_state.forgetts, this.gdpr_cookie_expire, this.fpcdomain, this.fpdata_sec_state, false);
		};
		this.try_and_save_gdpr_state = function() {
			try {
				if (this.gdpr_state_change.optout != null || this.gdpr_state_change.forgetts != null) {
					if (this.fpcdomain != '') {
						var gdpr_state = this.get_gdpr_state();
						if (this.gdpr_state_change.optout != null && this.gdpr_state_change.optoutts >= gdpr_state.optoutts) {
							gdpr_state.optout = this.gdpr_state_change.optout;
							gdpr_state.optoutts = this.gdpr_state_change.optoutts;
						}
						if (this.gdpr_state_change.forgetts != null && this.gdpr_state_change.forgetts >= gdpr_state.forgetts) {
							gdpr_state.forgetts = this.gdpr_state_change.forgetts;
						}
						this.set_gdpr_cookie(gdpr_state);
						this.gdpr_state_change.optout = null;
						this.gdpr_state_change.optoutts = null;
						this.gdpr_state_change.forgetts  = null;
						this.check_privacy_limits();
					} else if (this.closing==0) {
						this.load_fpcdomain();
					}
				}
			} catch (e) {}
		};
		this.set_optout = function(optout) {
			this.gdpr_state_change.optout = optout?1:0;
			this.gdpr_state_change.optoutts = Math.floor((new Date()).getTime()/1000);
			this.try_and_save_gdpr_state();
		};
		this.gdpr_forget = function() {
			this.gdpr_state_change.forgetts = Math.floor((new Date()).getTime()/1000);
			this.try_and_save_gdpr_state();
		};
		this.parse_rudata = function(rudata) {
			var d = Math.floor((new Date()).getTime()/1000)+86400;
			var rd = {};
			var rudata_splitted = rudata.split('|');
			var len = rudata_splitted.length;
			rd.ruid = len > 0 ? rudata_splitted[0] : rudata;
			var rts = len > 1 ? parseInt(rudata_splitted[1]) : d;
			this.rudata.refreshts = (!rts || rts > d) ? d : rts;
			rd.type = len > 2 ? rudata_splitted[2] : '';
			var rint = len > 3 ? parseInt(rudata_splitted[3]) : 10;
			this.rudata.refresh_interval = (!rint || rint < 10) ? 10 : rint;
			var reset = len > 4 ? parseInt(rudata_splitted[4]) : 0;
			rd.reset = reset ? 1 : 0;
			rd.rudata_tail = len > 5 ? '|' + rudata_splitted.slice(5).join('|') : '';
			if (rd.reset || rd.type != this.rudata.type || (rd.type != 3 && rd.type != 4)) {
				this.rudata.ruid = rd.ruid;
				this.rudata.type = rd.type;
				this.rudata.reset = rd.reset;
				this.rudata.rudata_tail = rd.rudata_tail;
			}
			this.rudata.cached = (this.rudata.ruid != '') ? 1 : 0;
		};
		this.getrucookie = function() {
			var rucookie = gemius_cookie.get_on_state(this.rucookie_name, this.fpdata_sec_state);
			if (rucookie.sec_state!='unknown') {
				this.parse_rudata(rucookie.primary);
			}
			if (rucookie.err) {
				this.rudbg.err = rucookie.err;
			}
		};
		this.setrucookie = function() {
			if (this.fpdata_sec_state!='unknown' && this.fpcdomain!='') {
				gemius_cookie.set(this.rucookie_name, this.getrudata(), this.rucookie_expire, this.fpcdomain, this.fpdata_sec_state, false);
				this.rudbg.err = gemius_cookie.set(this.rucookie_pub_name, this.getruidpub(), this.rucookie_expire, this.fpcdomain, this.fpdata_sec_state, false);
			} else {
				this.rudbg.err = (this.fpcdomain == '') ? "no-domain" : "no-sec-state";
			}
		};
		this.getrudata = function() {
			return this.rudata.ruid + '|' + this.rudata.refreshts + '|' + this.rudata.type + '|' + this.rudata.refresh_interval + '|' + this.rudata.reset + this.rudata.rudata_tail;
		};
		this.getruidpub = function() {
			return this.rudata.ruid ? this.rudata.ruid : '-';
		};
		this.getrudataencoded = function() {
			if (typeof encodeURIComponent != 'undefined') {
				return encodeURIComponent(this.getrudata());
			} else {
				return this.rudata.ruid;
			}
		};
		this.clear_ruidto = function() {
			if (this.timeouts.ruidto!=null) {
				if (this.timeouts.ruidto.to!=null) {
					clearTimeout(this.timeouts.ruidto.to);
				}
				this.timeouts.ruidto = null;
			}
		};
		this.ruidto = function() {
			this.timeouts.ruidto = null;
			this.call_rudata_callbacks(null, "timeout");
		};
		this.get_ruid = function(callback) {
			this.rudata_callbacks[this.rudata_callbacks.length] = callback;
			this.try_and_load_rudata();
			this.try_and_call_rudata_callbacks();
			if (this.rudata_callbacks.length > 0) {
				this.clear_ruidto();
				this.timeouts.ruidto = this.set_timeout(this.bind(this.ruidto),10000);
			}
		};
		this.set_rudata = function(rudata) {
			if (this.check_privacy_limits()) {
				this.set_rudata_impl(rudata);
			}
		};
		this.set_rudata_impl = function(rudata) {
			this.getrucookie();
			this.parse_rudata(rudata);
			this.setrucookie();
		};
		this.call_rudata_callbacks = function(ruid, status) {
			this.clear_ruidto();
			for (var i=0 ; i<this.rudata_callbacks.length ; i++) {
				try {
					this.rudata_callbacks[i](ruid, {status:status});
				} catch (e) {}
			}
			this.rudata_callbacks = [];
			if (status == 'timeout') {
			...
			var rudata = this.getrudata();
			for (var id in this.rudata_frame_callbacks) {
				if (this.rudata_frame_callbacks.hasOwnProperty(id)) {
					try {
						this.rudata_frame_callbacks[id](ruid, status, rudata);
					} catch (e) {}
				}
			}
		};
		this.try_and_call_rudata_callbacks = function() {
			if (this.init_called && this.waiting_for_consent==0) {
				if (this.has_consent) {
					if (window.top !== window.self && this.ref.frametype != 2) {
						if (this.truid.status!='') {
							this.call_rudata_callbacks(this.truid.ruid, this.truid.status);
						}
					} else if (this.waiting_for_fpdata==0) {
						if (this.fpdata=='' || this.fpdata[0]=='-') {
							this.call_rudata_callbacks(null, "no-fpc");
						} else if (this.rudata.prefix=='') {
							this.call_rudata_callbacks(null, "no-access");
						} else if (this.rudata.cached && this.rudata.ruid!='') {
							if (this.rudata.ruid[0]!='-') {
								this.call_rudata_callbacks(this.rudata.ruid, "ok");
							} else {
								this.call_rudata_callbacks(null, "error");
							}
						}
					}
				} else {
		...
		this.try_and_load_rudata = function() {
			if ((window.top === window.self || this.ref.frametype == 2) && this.waiting_for_consent==0 && this.has_consent && this.waiting_for_fpdata==0 && this.fpdata!='' && this.fpdata[0]!='-') {
				var d = Math.floor((new Date()).getTime()/1000);
				if (this.rudata.refreshts<d) {
					this.parse_fpdata();
					if (this.rudata.prefix=='') {
						this.rudata_loaded();
						return;
					}
					var rts = this.rudata.refreshts;
					this.getrucookie();
					if (this.rudata.refreshts>rts) {
						this.rudata_loaded();
						return;
					}
					this.rudata.refreshts = d+this.rudata.refresh_interval;
					if (this.rudata.refresh_interval < 43200) {
						this.rudata.refresh_interval *= 2;
					}
					this.setrucookie();
					var domain = new String(document.location.hostname);
					var url = (this.hssl?'https://':'http://')+this.rudata.prefix+'.gemius.pl/getid.js?fpdata='+this.getfpdataencoded()+'&href='+domain;
					if (this.rudata.ruid && this.rudata.ruid[0] != '-') {
						url += '&rudata='+this.getrudataencoded();
						this.rudbg.get_type = this.rudata.type;
						if (this.rudbg.ini_ruid && this.rudbg.ini_ruid != this.rudata.ruid) {
							this.rudbg.diff = 1;
						}
					}
					url += '&brts=' + d;
					this.append_script(url,this.bind(this.rudata_loaded),0);
					if (!this.rudata.cached && !this.rudata.firstto) {
						this.waiting_for_rudata = 1;
						this.timeouts.rudatato = this.set_timeout(this.bind(this.rudatato),1000);
					}
					if (_is_prerender()) {
						this.prend_flags = this.prend_flags | 2;
					}
				}
			}
		};
		this.rudatato = function() {
			this.timeouts.rudatato = null;
			this.rudata.firstto = 1;
			this.waiting_for_rudata = 0;
			this.latehits();
		};
		this.rudata_loaded = function() {
			if (this.timeouts.rudatato!=null) {
				if (this.timeouts.rudatato.to!=null) {
					clearTimeout(this.timeouts.rudatato.to);
				}
				this.timeouts.rudatato = null;
			}
			if (_is_prerender()) {
				this.prend_flags = this.prend_flags | 4;
			}
			this.try_and_call_rudata_callbacks();
			this.waiting_for_rudata = 0;
			this.latehits();
		...
		this.clear_deprecated = function() {
			try {
				if (typeof localStorage != 'undefined' && localStorage != null) {
					localStorage.removeItem('gemius_ruid');
				}
			} catch (e) {}
		};
		this.init = function(params) {
			...
			if (this.dnt==0 && this.explicit_consent!==false) {
				...
				if (this.waiting_for_fpdata==0) {
					this.getrucookie();
					this.fpsrc = 'cookie';
					if (this.rudata.ruid && this.rudata.ruid[0] != '-') {
						this.refresh_rucookie();
						this.rudbg.ini_ruid = this.rudata.ruid;
						this.rudbg.ini_type = this.rudata.type;
					}
				}
			} else {
				this.waiting_for_fpdata = 1;
			...
			this.require_consent_info = 0;
			if (this.check_privacy_limits() && this.dnt==0 && this.explicit_consent!==false) {
			...
			setInterval(this.bind(this.refresh_rucookie),10000);
			setInterval(this.bind(this.try_and_load_rudata),10000)
		};
		this.start = function() {
			if (this.started) {
				return;
			...
			if (_is_prerender()) {
				this.waiting_on_prerender = 1;
				this.prend_flags = this.prend_flags | 1;
...
})('gemius_hcconn', {'referrer_override':'referrer_override', 'get_ruid':'get_ruid', 'set_optout':'set_optout', 'gdpr_forget':'gdpr_forget', 'event': 'gcmdevent'});
